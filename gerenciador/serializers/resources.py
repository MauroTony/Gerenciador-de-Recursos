...
class ResourceSchedulingSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        get_resource = ResourceScheduling.objects.filter(
            resource_id=resource_id, final_date__gte=date_ini, is_deleted=False
        )
        if get_resource:
            for resource in get_resource:
                if resource.initial_date <= date_ini <= resource.final_date:
                    raise serializers.ValidationError(
                        {"msg": "Resource already scheduled in this period"}
                    )
        return data
    ...
    def delete(self, instance, user):
        if instance.user == user or user.is_staff:
            ...
            raise serializers.ValidationError(
                {"msg": "You can only delete your own reservations"}
            )
        return instance
