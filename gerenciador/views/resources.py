...
from ..serializers import ResourcesSerializer, ResourceSchedulingSerializer
...
class ResourceScheduleView(APIView):
    ...
    def post(self, request):
        ...
        if (
            ...
            or "initial_date" not in data
            or "final_date" not in data
        ):
            return Response({"msg": "missing data"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            if not resource.available:
                return Response(
                    {"msg": "Resource unavailable"}, status=status.HTTP_400_BAD_REQUEST
                )
            print("data: ", data_dict)
            serializer = ResourceSchedulingSerializer(data=data_dict)
            serializer.is_valid(raise_exception=True)
            print("valido")
            serializer.create()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
